...
`local/compose.yaml` starts MySQL and WordPress as two containers, with a named volume
...
thing exposed to the host, on port 80. Supplied uploads are mounted read-only
from outside the image, matching the cloud deployment's separation of code and
media.
...
[`local/checklist.md`](local/checklist.md), which lists what to look at
